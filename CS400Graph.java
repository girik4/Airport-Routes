...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        if (start == null || end == null)
            throw new NoSuchElementException("Cannot compute path with null start or end");
        ...
        class NewVertex {
            ...
            public NewVertex(T data, Vertex v) {
                this.data = data;
                this.edgesLeaving = new LinkedList<>();
                for (int i = 0; i < v.edgesLeaving.size(); i++)
                    this.edgesLeaving.add(v.edgesLeaving.get(i));
                this.visited = false;
                this.weight = Integer.MAX_VALUE;
                this.predecessor = new Path(v);
            }
        }

        Hashtable<T, NewVertex> verticesCopy = new Hashtable<>();

        Enumeration<T> keys = vertices.keys();
        while (keys.hasMoreElements()) {
            T currentData = keys.nextElement();
            verticesCopy.put(currentData, new NewVertex(currentData, vertices.get(currentData)));
        }

        verticesCopy.get(start).weight = 0;


        PriorityQueue<Path> pq = new PriorityQueue();
        pq.add(new Path(startVertex));
        ...
        while (!pq.isEmpty()) {
            currentPath = pq.poll();
            currentVertex = currentPath.end;
            verticesCopy.get(currentVertex.data).visited = true;

            NewVertex successor;
            for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) {
                successor = verticesCopy.get(currentVertex.edgesLeaving.get(i).target.data);
                if (successor.visited == false) {
                    Path potentialNewPath = new Path(currentPath, currentVertex.edgesLeaving.get(i));
                    if (successor.weight > potentialNewPath.distance) {
                        successor.weight = potentialNewPath.distance;
                        successor.predecessor = potentialNewPath;
                        pq.add(successor.predecessor);
                        foundPaths.add(potentialNewPath);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < foundPaths.size(); i++) {
            if (foundPaths.get(i).start.equals(startVertex) && foundPaths.get(i).end.equals(endVertex))
                finalPath = foundPaths.get(i);
        }
